...
use livekit_gstreamer::{
    initialize_gstreamer, AudioPublishOptions, GstMediaStream, LKParticipant, LocalFileSaveOptions, PublishOptions, RtspStream, RtspStreamType, VideoPublishOptions
};
...
async fn register_device(
    ...
    let device_request = DeviceRegisterRequest {
        comments: Some("Vaddio Publisher Registration".into()),
        group: "A2004RoomCameras".into(),
        name: "VaddioAV Bridge".into(),
    };
...
async fn publish_streams(
    ...
    let tk_request = TokenRequest {
        identity: "vaddio".to_string(),
        name: Some("A2004 Vaddio Publisher".to_string()),
    ...
    let op_dir = format!("recordings-vaddio-a2004/{}-{}", session_name, session_id);
    ...
    log::info!("Creating RTSP audio stream: {} channels @ 48000Hz", selected_channels.len());

    // participant.publish_rtsp_stream(&mut streams[0], None).await?;
    for stream in streams.iter_mut() {
        stream.start().await?;
        participant
            .publish_rtsp_stream(stream, Some(format!("vaddio-camera").into()))
            .await?;
    }
