...
use livekit_gstreamer::{
    initialize_gstreamer, GstMediaStream, LKParticipant, LocalFileSaveOptions, PublishOptions,
    VideoPublishOptions,
};
...
async fn register_device(
    ...
    let device_request = DeviceRegisterRequest {
        comments: Some("Registering device for livekit-gstreamer example".into()),
        group: "test-group".into(),
        name: "GStreamer Device".into(),
    };
...
async fn publish_streams(
    ...
    let tk_request = TokenRequest {
        identity: "gstreamer-bot".to_string(),
        name: Some("GStreamer Bot".to_string()),
